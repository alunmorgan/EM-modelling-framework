...
for nes = 1:length(analysis_loc)
    if exist(input_list{1}, 'file') == 2
        ...
        plot_s_param_graph(sparameter_data, modelling_inputs.beam, cols_sep, fig_pos, output_folder, lower_cutoff, linewidth)
        plot_s_parameter_reflection_graph(sparameter_data, modelling_inputs.beam, cols_sep, lines, fig_pos, output_folder, lower_cutoff, linewidth)
        ...
        if strcmp(modelling_inputs.beam, 'yes') && length(receivers) > 2
            plot_s_parameter_transmission_graphs_port(sparameter_data, modelling_inputs.beam, cols_sep, lines, fig_pos, output_folder, lower_cutoff, linewidth)
            plot_s_param_transmission_graphs_modes(sparameter_data, modelling_inputs.beam, cols_sep, lines, fig_pos, output_folder, lower_cutoff, linewidth)
