function plot_s_parameter_impedance_graph(data, cols, lines, fig_pos, ...
...
for nre = 1:length(data.port.names) % excitation ports
        ...
        y_data = data.port.impedances{nre};
...
ylabel('Port Impedance')
title('Port Impedance')
savemfmt(h, pth,[prefix, '_s_parameters_impedance'])
close(h)