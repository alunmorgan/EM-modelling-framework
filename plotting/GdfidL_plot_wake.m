...
% Extracting time series
 ...
 cut_time_ind = find(timebase * 1E-9 <= str2double(chosen_wake_length)/3E8, 1, 'last');

[wp, wpdx, wpdy, wpqx, wpqy] = extract_wake_potential_from_wake_data(wake_data);

 [e_total_cs, e_ports_cs] =  ...
    extract_cumulative_total_energy_from_wake_data(wake_data);

[modes, max_mode, dominant_modes, port_mode_energy] = ...
    extract_port_signals_from_wake_data(wake_data);

% Extracting spectra
frequency_scale = wake_data.frequency_domain_data.f_raw(1:cut_freq_ind)*1E-9;
...
%% Port signals
    plot_port_signals(h_wake, path_to_data, ...
        ...
        dominant_modes,port_names, timebase, modes)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
report_plot_frequency_graphs(fig_pos, path_to_data, y_lev_f, frequency_scale, y_data, ...
...
if isfield(pp_data.port, 'timebase')
    ...
    report_plot_frequency_graphs(fig_pos, path_to_data, y_lev_f,...
        frequency_scale, power_diff, cut_freq_ind, cuts_temp,...
...
if wake_data.time_domain_data.port_data.power_port_mode.full_signal.total_energy ~=0
    ...
    if bp_only_flag == 0
        plot(frequency_scale, signal_port_spectrum,'r',...
            frequency_scale, beam_port_spectrum,'k','LineWidth',lw)
    ...
    % multiply by 2.
    if bp_only_flag == 0
        plot(frequency_scale, cumsum(signal_port_spectrum) .*2,'r',...
            frequency_scale, cumsum(beam_port_spectrum).*2,'k','LineWidth',lw)
    ...
    hold(ax(18), 'on')
        for ns = 1:size(port_spectra,1)
            plot(frequency_scale, port_spectra(ns,:),'LineWidth',lw, 'DisplayName', port_names{ns})
        end %for
    hold(ax(18), 'off')
    graph_add_vertical_lines(cuts_temp)
    ...
    hold(ax(19), 'all')
        for ns = 1:size(port_spectra,1)
            plot(frequency_scale,...
                cumsum(port_spectra(ns,:)).*2,'LineWidth',lw, 'DisplayName', port_names{ns})
        end %for
    hold(ax(19), 'off')
    graph_add_vertical_lines(cuts_temp)
...
% hold(ax(27), 'on')
% for mers = 1:size(peaks_end_ts,1)
%     leg = [num2str(round(peaks_end_ts(mers,1) .* 10)./10), ' GHz'];
%     plot(peaks_ts(mers,1), peaks_end_ts(mers,2),'*r','LineWidth',lw, 'DisplayName', leg)
% end %for
% hold(ax(27), 'off')
% xlabel('Frequency (GHz)')
...
% for wana = 1:size(peaks_start_ts,1)
%     if wana >1
%         hold(ax(28), 'all')
%     end %if
%     f_ind = frequency_scale_ts == peaks_start_ts(wana,1);
...
%     tau =  - x2 ./(y2 - y1);
%     Q_graph = pi .* peaks_start_ts(wana,1)*1E9 .* tau;
%     leg = [num2str(round(peaks_start_ts(wana,1) .* 10)./10), ' GHz   :Q: ',num2str(round(Q_graph))];
...
% This is to make sure that enough modes were used in the simulation.
    ...
    for ydh = 1:length(port_names) % Ports
        x_vals = 1:size(port_mode_energy,2);
        subplot(hwn,ksn,ydh)
        plot(x_vals, port_mode_energy(ydh,:),'LineWidth',lw);
        xlabel('mode number')
    ...
    if ~isnan(minlim)
        if minlim >0
            semilogy(energy(:,1),energy(:,2),'b', 'LineWidth',lw, 'DisplayName', 'Energy decay')
                hold(ax(33), 'on')
                semilogy(timebase, squeeze(e_total_cs(:)) * 1e9,':k',...
...
%% Checking the cumsum scaling
    ...
    data = beam_port_spectrum + signal_port_spectrum; %NEEDS TO BE ENERGY
    plot(frequency_scale, cumsum(data).*1E9,':k')
    hold(ax(37), 'on')
    plot([frequency_scale(1), frequency_scale(end)],...
        [total_port_energy_loss, total_port_energy_loss],'r')
    plot([frequency_scale(floor(end/2)), frequency_scale(floor(end/2))],...
        [0, total_port_energy_loss],':c')
    hold(ax(37), 'off')
...
plot(frequency_scale,cumsum(wake_data.frequency_domain_data.Bunch_loss_energy_spectrum(1:cut_freq_ind))*1e9,':k','LineWidth',lw)
hold(ax(38), 'on')
plot([frequency_scale(1), frequency_scale(end)],...
...
plot(pp_data.Wake_potential(:,1)* 1E9,pp_data.Wake_potential(:,2) ./ max(abs(pp_data.Wake_potential(:,2))),'b',...
    timebase, wake_data.time_domain_data.wakepotential ./ max(abs(pp_data.Wake_potential(:,2))),'.c',...
    pp_data.Charge_distribution(:,1) * 1E9,pp_data.Charge_distribution(:,2) ./ max(pp_data.Charge_distribution(:,2)),'r',...
    timebase, wake_data.time_domain_data.charge_distribution ./ max(wake_data.time_domain_data.charge_distribution),'.g',...
    'LineWidth',lw)
hold(ax(39), 'on')
