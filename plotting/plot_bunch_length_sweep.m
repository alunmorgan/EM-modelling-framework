...
hold on
for jse = 1:length(bunch_length_sweep_data.Bunch_loss_energy_spectrum)
    %FIXME add frequency scale once reconstruction is fixed.
    plot(bunch_length_sweep_data.Bunch_loss_energy_spectrum{jse}, 'LineWidth',lw,...
        'Parent', ax, 'DisplayName', ...
        ['Bunchlength = ', num2str(round(bunch_length_sweep_data.sig_time(jse)*3e8*1e-3)), 'mm']);
end %for
