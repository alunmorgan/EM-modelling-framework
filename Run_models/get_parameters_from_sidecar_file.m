...
for nse = 1:size(vals, 1)
    ...
    if ~isempty(strfind(vals{nse, 2}, '['))
        ...
        if ~isempty(delims)
            tmp(1) = convert_to_Gdfidl_units(vals{nse, 2}(list_begin+1:delims(1)-1));
            for ns = 1: length(delims) -1
                
                tmp(ns+1) = convert_to_Gdfidl_units(vals{nse, 2}(delims(ns)+1:delims(ns+1)-1));
            end %for
            tmp(ns +2) = convert_to_Gdfidl_units(vals{nse, 2}(delims(ns + 1)+1:list_end-1));
        else
            tmp = [];
        end %if
        parameters{nse}{2} = {tmp};
    else
        parameters{nse}{2} = {convert_to_Gdfidl_units(vals{nse, 2})};
    end
    parameters{nse}{3} = '';
end %if
end %function

function out = convert_to_Gdfidl_units(val)
% Args:
%         val(str): parameter from the parameter file.
if contains(val, 'mm')
