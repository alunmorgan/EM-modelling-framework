function model_file = create_cut_plot_plots (modelling_inputs, eyepos, geom_prefix, out_loc)
model_file = {'-cutplot'};
...
for ndw = 1:size(modelling_inputs.cuts,1)
    ...
    model_file = cat(1, model_file, ['   plotopts = -o ', fullfile(out_loc, [geom_prefix, '2Dmodel_cutplot_',modelling_inputs.cuts{ndw, 1},'_user',num2str(count),'.ps']), ' -colorps']);
