...
% modelling_inputs is
%
% Example:  arch_date = run_eigenmode_simulation(mi, modelling_inputs, arch_date)
...
temp_files('make')
construct_eigenmode_gdf_file(mi, modelling_inputs, 'no')
...
if ~exist([storage_path, output_name,'/',arch_date,'/eigenmode'],'dir')
    mkdir([storage_path, output_name,'/',arch_date], 'eigenmode')
end

arch_out = [storage_path, output_name,'/',arch_date,'/eigenmode/'];
movefile('temp_data/*', arch_out);
copyfile([mi.input_file_path, mi.model_name, '_model_data'], [storage_path, output_name,'/',arch_date,'/eigenmode/']);
