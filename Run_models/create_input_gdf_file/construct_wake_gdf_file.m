function construct_wake_gdf_file(out_loc, modelling_inputs, paths, tail)
...
fs = gdf_wake_header_construction(paths, tail, ...
...
mesh_fixed_planes = gdf_write_mesh_fixed_planes(modelling_inputs.beam_offset_x, ...
    modelling_inputs.beam_offset_y);
data = create_model_data_file_for_STL(modelling_inputs, 'wake', out_loc);
...
if isfield(modelling_inputs, 'voltage_monitoring')
    mon = gdf_wake_monitor_construction(modelling_inputs.dtsafety,...
        modelling_inputs.voltage_monitoring);
else
    mon = gdf_wake_monitor_construction(modelling_inputs.dtsafety, cell(0,0));
end %if
...
elseif isfield(modelling_inputs.field_capture, 'full_field_snapshot_times')
    field_ss = gdf_wake_field_snapshots(modelling_inputs.field_capture);
else
    field_ss = {' '};
end %if
...
data = cat(1,fs, modelling_inputs.defs', geom, mesh_def, mesh_fixed_planes, ...
    data, port_defs, excitation, vs,  mon, field_ss, '-fdtd   ','    doit');
