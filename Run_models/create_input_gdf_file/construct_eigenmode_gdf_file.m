function construct_eigenmode_gdf_file(out_loc, modelling_inputs, paths, tail, islossy)
...
if ~isempty(modelling_inputs.mat_list)
    ...
    material_override = find_material_overrides(materials, ...
        modelling_inputs.defs);
else
    material_labels = [];
    material_override = [];
end %if

fs = gdf_eigenmode_header_construction(paths, tail,...
...
mon = gdf_eigenmode_monitor_construction(...
    ...
    modelling_inputs.eigenmode.passes);
