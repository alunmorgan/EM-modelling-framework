...
for hse = length(good_field_data):-1 :1
    if good_field_data(hse) == 1
        ...
        for ens = 1:length(report_input.field_snapshot_times)
            data_out(ck, ens, :) = blend_field_data(field_data, report_input.field_snapshot_times(ens));
        end %for
        data_out(ck, ens +1, :) = blend_field_data(field_data, 'max');
        graph_metadata.sources(ck) = report_input.sources(hse);
        graph_metadata.swept_vals(ck) = report_input.swept_vals(hse);
        ck = ck +1;
    end %if
...
if sum(good_field_data) > 0
    for fns = 1:length(report_input.field_snapshot_times) + 1
        for egvn = 1:size(data_out,2)
            Generate_2D_graph_with_legend(graph_metadata, squeeze(data_out(:,fns, egvn)), cols, l_st)
        end %for
    end %for
end %if