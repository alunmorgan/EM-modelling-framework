function Blend_figs(report_input, sub_folder , chosen_wake_length)
...
for hse = length(good_data):-1 :1
    if good_data(hse) == 1
        ...
        data_out(ck, 2).ydata = wake_sweep_data.frequency_domain_data{chosen_wake_ind}.Wake_Impedance_data;
        data_out(ck, 2).xdata = wake_sweep_data.frequency_domain_data{chosen_wake_ind}.f_raw' * 1E-9;
        ...
        data_out(ck, 3).ydata = wake_sweep_data.frequency_domain_data{chosen_wake_ind}.Wake_Impedance_data_im;
        data_out(ck, 3).xdata = wake_sweep_data.frequency_domain_data{chosen_wake_ind}.f_raw' * 1E-9;
