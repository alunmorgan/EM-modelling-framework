...
if exist('data_link/wake/', 'dir')
    ...
    try
        ...
        % Generate the plots.
        GdfidL_plot_wake(pp_data.wake_data, ppi, modelling_inputs, run_logs.wake,'pp_link/wake/', 1E7)
    catch W_ERR
        display_postprocessing_error(W_ERR, 'wake')
    end
end
%% Post processing S-parameters
if exist('data_link/s_parameters', 'dir')
    % Creating sub structure.
    try
...
if exist('data_link/eigenmode/', 'dir')
    % Creating sub structure.
...
if exist('data_link/lossy_eigenmode/', 'dir')
    % Creating sub structure.
