...
if exist(fullfile('data_link', 'eigenmode/'), 'dir')
    ...
    % Save input structure
    save(fullfile('pp_link', 'eigenmode', 'pp_inputs.mat'), 'ppi');
    % Move files to the post processing folder.
    ...
    run_logs = GdfidL_read_eigenmode_log(...
        fullfile('pp_link', 'eigenmode', 'model_log'));
    save(fullfile('pp_link', 'eigenmode', 'data_from_run_logs.mat'), 'run_logs')
    
    % Running postprocessor
    disp('GdfidL_post_process_models: Post processing eigenmode data.')
    orig_ver = getenv('GDFIDL_VERSION');
    pp_data = postprocess_eigenmode(ppi);
    % restoring the original version.
    setenv('GDFIDL_VERSION',orig_ver)
...
if exist(fullfile('data_link', 'lossy_eigenmode/'), 'dir')
    ...
    % Save input structure
    save(fullfile('pp_link', 'lossy_eigenmode', 'pp_inputs.mat'), 'ppi');
    % Move files to the post processing folder.
    ...
    run_logs = GdfidL_read_eigenmode_log(...
        fullfile('pp_link', 'lossy_eigenmode', 'model_log'));
    save(fullfile('pp_link', 'lossy_eigenmode', 'data_from_run_logs.mat'), 'run_logs')
    disp('GdfidL_post_process_models: Post processing lossy eigenmode data.')
    
    % Running postprocessor
    orig_ver = getenv('GDFIDL_VERSION');
    pp_data = postprocess_eigenmode_lossy(ppi);
    % restoring the original version.
    setenv('GDFIDL_VERSION',orig_ver)
    save(fullfile('pp_link', 'lossy_eigenmode', 'data_postprocessed.mat'), 'pp_data','-v7.3')
...
if exist(fullfile('data_link', 'shunt'), 'dir')
    try
        ...
        % Save input structure
        save(fullfile('pp_link', 'shunt', 'pp_inputs.mat'), 'ppi');
        ...
        for ief = 1:length(out)
            if  ~isempty(strfind(out{ief},'model_log')) && ~isempty(strfind(ppi.solvers, 'r'))
                run_logs = GdfidL_read_rshunt_log(fullfile('pp_link', 'shunt', out{ief}));
                break
            end
        end
        
        save(fullfile('pp_link', 'shunt', 'data_from_run_logs.mat'), 'run_logs')
        % Running postprocessor
        ...
        if  ~isempty(strfind(ppi.sim_select, 'r'))
            ...
            if tst == 1
                ...
                GdfidL_plot_shunt(pp_data, fullfile('pp_link', 'shunt'))
            else
                pp_data = NaN;
            end
        else
        ...
        save(fullfile('pp_link', 'shunt', 'data_postprocessed.mat'), 'pp_data')
