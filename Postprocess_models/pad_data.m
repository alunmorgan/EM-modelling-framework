...
elseif length(new_lengths) == 2
    ...
    warning('Too many lengths to pad data with')
end
...
if strcmp(length_type,'points')
    ...
    if iscell(input_data)
        for rn = 1:length(input_data) %number of sets.
            ...
            %             end %for
            [padded_output{rn}, timescale_temp] = pad_vector(input_data{rn}, timescale, 'pos', new_length);
        ...
        %             end %if
        %         end %for
        [padded_output, timescale_out] = pad_vector(input_data, timescale, 'pos', new_length);
        %         padded_output = tmp;
        %         clear tmp
    end %if
