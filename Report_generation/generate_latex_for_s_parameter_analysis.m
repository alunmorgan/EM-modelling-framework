...
[s_list,~] =  dir_list_gen([data_path,'/s_parameter'],'eps',1);
...
t = regexp(s_params, 's_parameters_S(\d)(\d{1,2})\.eps', 'tokens');
...
if n_ports >2
    ...
    for newq = 1:4:len_pe
        ov1 = latex_side_by_side_images(['s_parameter/',port_excitations{newq}],...
            ['s_parameter/', port_excitations{newq+1}],'','');
        ov = cat(1,ov,ov1);
        ov1 = latex_side_by_side_images(['s_parameter/', port_excitations{newq+2}],...
            ['s_parameter/', port_excitations{newq+3}],'','');
        ov = cat(1,ov,ov1);
        ov = cat(1,ov,'\clearpage');
    end %for
end %if
...
for newq = 1:4:len_s
    if newq +3 < len_s
        ov1 = latex_side_by_side_images(['s_parameter/',s_params{newq}],...
            ['s_parameter/', s_params{newq+1}],'','');
        ov = cat(1,ov,ov1);
        ov1 = latex_side_by_side_images(['s_parameter/', s_params{newq+2}],...
            ['s_parameter/', s_params{newq+3}],'','');
        ov = cat(1,ov,ov1);
        ov = cat(1,ov,'\clearpage');
    else
        extras = 4 - mod(len_s,4);
