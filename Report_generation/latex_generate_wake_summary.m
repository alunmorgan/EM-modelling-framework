function combined = latex_generate_wake_summary(wake_data, mi, ppi)
...
sb_Values_list{1} = num2str(round(mi.bunch_charge *1E9));
sb_Settings_list{2} = 'Bunch length (ps)';
sb_Values_list{2} = num2str(round(str2double(mi.beam_sigma) ./3E8 * 1E12*10)/10);
...
if isfield(wake_data, 'mat_losses')
    sb_Values_list{5} = ' ';%[num2str(round(wake_data.mat_losses.total_loss(end)/wake_data.frequency_domain_data.Total_bunch_energy_loss * 100)),'\%'];
else
...
combined = cat(1,combined, '\begin{tabular}{|p{0.4\textwidth}|}');
combined = cat(1,combined, '\hline');
combined = cat(1,combined, ['Wakeloss factor V/pC)','\\']);
combined = cat(1,combined, '\hline');
combined = cat(1,combined, [num2str(round(wake_data.wake_loss_factor * 1E-12*100)/100),'\\' ]);
combined = cat(1,combined, '\hline');
