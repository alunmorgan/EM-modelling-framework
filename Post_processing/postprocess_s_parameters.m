function postprocess_s_parameters(model_name)
...
[s_names, pth] = dir_list_gen(fullfile('data_link', 'sparameter'), 'dirs', 1);
...
for osw = 1:length(s_names)
    ...
    s_parameter_output_directory = fullfile('pp_link', 'sparameter', s_names{osw});
    ...
    pp_output_name = fullfile(s_parameter_output_directory ,['model_s_param_set_',s_set, '_',excite,'_post_processing_input_file']);
    write_out_data(pp_s_input_file, pp_output_name )
    
    postprocess_core(s_parameter_output_directory, modelling_inputs.version, 'sparameter', s_set, excite, 'pp_input_file', pp_output_name);
end