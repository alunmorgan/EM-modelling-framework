...
[s_names, pth] = dir_list_gen('data_link/s_parameter/', 'dirs', 1);
...
for osw = 1:length(s_names)
    ...
    excite = excite{1}{1};
        [~] = system(['mkdir ', fullfile('pp_link', 's_parameter',['model_s_param_',excite,'_post_processing'])]);
    s_port{osw} = excite;
    GdfidL_write_pp_s_param_input_file(excite)
    temp_files('make')
    [~]=system(['gd1.pp < ', ...
        fullfile('pp_link', 's_parameter', ['model_s_param_',excite,'_post_processing'], ['model_s_param_',excite,'_post_processing_input_file']), ' > ',...
        fullfile('pp_link', 's_parameter', ['model_s_param_',excite,'_post_processing'], ['model_s_param_',excite,'_post_processing_log'])]);
