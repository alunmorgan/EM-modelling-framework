...
def make_field_images(
    data, metadata, timestamps, max_field_component, output_location, name_of_model, ROI
):
    ...
    if not np.isnan(ROI):
        ...
        dirfields_trim = {
            field: data[field][x_ind1 : x_ind2 + 1, y_ind1 : y_ind2 + 1, :]
    ...
    if not os.path.isfile(os.path.join(output_location, out_name)):
        ...
        for oas, field_name in enumerate(field_components):
            ...
            graph_lims = [
                np.nanmin(dirfields_trim_temp),
                np.nanmax(dirfields_trim_temp),
            ]
            timescale = timestamps
            Frames = []
            for ifen in tqdm(
                range(dirfields_trim_temp.shape[2]), desc=f"Processing {field_name}"
            ):
        ...
        plt.close(fig)
        sio.savemat(
            os.path.join(output_location, out_name), {"field_images": field_images}
        )
        return field_images


def plot_z_slice_fields(
    ax, xaxis, yaxis, slice_data, slice_dir, field_name, actual_time, graph_lims
):
    c = ax.pcolormesh(
        xaxis, yaxis, slice_data, shading="auto", vmin=graph_lims[0], vmax=graph_lims[1]
    )
...
def plot_field_slices(
    ...
    main_title_string = (
        f"{field_type}-field slice direction: {slice_dir} at {time_val}ns"
    )
    ...
    temp_accum = np.full(
        (data_Fx.shape[0], data_Fx.shape[1], len(field_components)), np.nan
    )
    ...
    for igr, field_name in enumerate(field_components):
        ...
        if field_levels is None:
            ...
            contour = ax.contourf(
                xaxis * 1e3, yaxis * 1e3, slice_data.T, levels=field_levels, cmap="jet"
            )
    ...
    if field_levels is None:
        ...
        contour = ax.contourf(
            xaxis * 1e3, yaxis * 1e3, temp_accum.T, levels=field_levels, cmap="jet"
        )
...
def plot_field_views_selected_timeslice(
    data, field_type, slice_dir, output_location, name_of_model, selected_time, ROI
):
    field_components = ["Fx", "Fy", "Fz"]
    ...
    for field_name in field_components:
        ...
        if not os.path.isfile(os.path.join(output_location, f"{output_name}.png")):
            ...
            geometry_slice_trim = geometry_slice[
                x_ind1 : x_ind2 + 1, y_ind1 : y_ind2 + 1
            ]
...
def plot_z_slice_fields(
    ax, xaxis, yaxis, slice_data, slice_dir, field_name, field_type, actual_time
):
