...
for kfn = 1:length(extracted_data)
    ...
    base_ind = contains(names, 'Base');
         makeBaseSummaryTable(extracted_data{kfn}, model_sets{kfn}, root, base_ind)
    for hen = 1:length(sweeps)
        ...
        makeSweepSummaryTables(valsX, dataInds, extracted_data{kfn}, model_sets{kfn}, xunit, sweeps{hen}, root)
        makeSweepSummaryGraphs(valsX, dataInds, base_val, base_ind, extracted_data{kfn}, model_sets{kfn}, xunit, sweeps{hen}, root)
